...
| --- | --- | --- | --- |
| [Spring Modulith](https://github.com/spring-projects/spring-modulith) | Apache-2.0 | **当前不引入** | 能解决持久事件问题，但会扩大 #55 范围。V1 先复用现有 Spring 事件、ClientEvent 和 Push 边界；只有真实出现不可接受的丢失窗口后再评估最小 outbox。 |
...
3. #55 不增加开源框架；先完成最小业务通知闭环、`AFTER_COMMIT`、稳定 `eventId` 和幂等去重。可靠性升级独立立项，不阻塞 V1。
