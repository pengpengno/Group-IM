...
class UserControllerSpec extends Specification {
    ...
    def "测试根据用户ID查询用户成功场景"() {
        ...
        def userId = 1L
        def currentUser = new User()
        currentUser.setUserId(2L) // 当前登录用户ID为2
    ...
    def "测试空指针异常处理"() {
        ...
        then: "验证空指针异常处理"
        1 * userService.findUserByUsername(null) >> { throw new NullPointerException("用户名不能为空") }
    ...
    def "测试超长用户名查询"() {
        given: "准备超长用户名查询"
        def longUsername = "a" * 1000 // 1000个字符的用户名
